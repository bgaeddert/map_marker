    ...
    >
      <MarkerCluster v-if="shouldCluster">
        <Marker v-for="(location, i) in locations"
                @click="(e) => handleMarkerClick(e, location.idx)"
                :options="{ position: location.position, icon: pinSymbol(location.type) }"
                :key="i"
        >
          <InfoWindow ref="otherInfoWindows" @visible="handleInfoVisible('otherInfoWindows', i)" @closeclick="handleCloseInfoWindow">
      ...
      <Marker v-for="(store, i) in stores"
              @click="(e) => handleMarkerClick(e, store.idx)"
              :options="{ position: store.position, icon: pinSymbol(store.type) }"
              :key="i"
      >
        <InfoWindow ref="storeInfoWindows" @visible="handleInfoVisible('storeInfoWindows', i)" @closeclick="handleCloseInfoWindow">
      ...
      <div v-if="!shouldCluster">
        <Marker v-for="(location, i) in locations"
                ...
                :key="i"
        >
          <InfoWindow ref="clusteredOtherInfoWindows" @visible="handleInfoVisible('clusteredOtherInfoWindows', i)" @closeclick="handleCloseInfoWindow">
            <info-content :location="location"/>
          </InfoWindow>
        </Marker>
      </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    handleMarkerClick: function (event, idx) {
    ...
    handleIdle: function (){
      ...
      let boundLocations = this.locations.map((l, i) => {return {...l, posIdx: i};}).filter((l) => {
